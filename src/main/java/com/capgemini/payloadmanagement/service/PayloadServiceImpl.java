	...
	public JsonObject mapPayload(JsonObject json, Map<String, Object> request, Integer arrayIteration) {
		...
		json.entrySet().parallelStream().forEach(entry -> {
			...
			if (value.isJsonObject() && value.getAsJsonObject().get("type").getAsString() != null) {
					...
					if (value.getAsJsonArray().get(0).getAsJsonObject().get("type").getAsString().equals("Map")) {
						try {
							...
							list.stream().forEach(e -> {
								array.add(mapPayload(JsonParser.parseString(new Gson().toJson(new Gson().fromJson(
										new Gson().toJson(value.getAsJsonArray().get(0).getAsJsonObject()), Map.class)))
										.getAsJsonObject(), request, counter[0]));
								counter[0]++;
							});
							...
							if (ex.getMessage().toString().contains(
								...
								throw new BadRequestException(ex.getMessage().toString().replace("com.google.gson.", "")
										+ " from "
				...
				case "merge": {
					try {
						json.add(entry.getKey(), getJsonArrayFromRequest(
								value.getAsJsonArray().get(0).getAsJsonObject().get("path").getAsString(), request));
					} catch (IllegalStateException ex) {
				...
				case "polling": {
					...
					value.getAsJsonArray().asList().stream().forEach(element -> {
						try {
							element.getAsJsonObject().remove("type");
							element.getAsJsonObject().remove("mappingType");
							element.getAsJsonObject().remove("path");
							array.add(mapPayload(JsonParser
									.parseString(new Gson().toJson(new Gson()
											.fromJson(new Gson().toJson(element.getAsJsonObject()), Map.class)))
									.getAsJsonObject(), request, null));
	...
	public Boolean validateRequest(JsonObject json, Integer arrayIteration, String arrayKey) {
		...
		json.entrySet().parallelStream().forEach(entry -> {
			...
			if (value.isJsonObject()) {
				if (value.getAsJsonObject().get("path") == null
					...
					if (value.getAsJsonObject().get("type") == null)
						throw new BadRequestException("type is required for the key: " + entry.getKey()
								+ ((arrayIteration != null) ? " in " + arrayKey + "[" + arrayIteration + "]" : ""));
					else if (!Arrays.asList(allowedTypes).contains(value.getAsJsonObject().get("type").getAsString()))
				...
				int[] counter = new int[1];
				value.getAsJsonArray().asList().stream().forEach(e -> {
					...
					if (e.getAsJsonObject().get("path") == null
						...
						if (e.getAsJsonObject().get("mappingType") == null)
							throw new BadRequestException(
									"mappingType is required for the key: " + entry.getKey() + "[" + counter[0] + "]");
						else if (!Arrays.asList(allowedMappingTypes)
								.contains(e.getAsJsonObject().get("mappingType").getAsString()))
							throw new BadRequestException(
									"Invalid mappingType -> " + e.getAsJsonObject().get("mappingType").getAsString()
						 ...
						 if (e.getAsJsonObject().get("type") == null
						...
						else if (!(e.getAsJsonObject().get("mappingType").getAsString().equals("merge"))&& !Arrays.asList(allowedTypes).contains(e.getAsJsonObject().get("type").getAsString()))
							throw new BadRequestException(
									"Invalid type -> " + e.getAsJsonObject().get("type").getAsString()
											+ " <- provided for key: " + entry.getKey() + "[" + counter[0] + "]");
							...
							throw new BadRequestException(e.getAsJsonObject().get("type").getAsString()
									+ " type is not allowed for handshake mappingType for the key: " + entry.getKey()
									+ "[" + counter[0] + "]");
						else {
